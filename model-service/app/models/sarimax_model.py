"""
SARIMAX forecasting model with external regressors
"""
...
class SARIMAXForecaster:
    ...
    def get_params(self) -> Dict[str, Any]:
        return {
            ...
            "order": "(1,1,1)",
            "seasonal_order": f"(1,1,1,{self.seasonal_period})"
        }
    ...
    def forecast(
        ...
        for cliente in sales_data.clientes:
            ...
            for sku in cliente.skus:
                ...
                if len(historico) < self.seasonal_period or sum(historico) == 0:
                    forecast_detail = [0] * forecast_months
                    mape = 999
                else:
                    try:
                        # Prepare series
                        ts = pd.Series(historico, index=pd.date_range(
                            start=meses_fecha[0],
                            periods=len(historico),
                            freq='MS'
                        ))
                        ...
                        if len(ts) < self.seasonal_period:
                            forecast_detail = self._simple_forecast(historico, forecast_months)
                            mape = 999
                        else:
                            # Fit SARIMAX
                            model = SARIMAX(
                                ts,
                                order=(1, 1, 1),
                                seasonal_order=(1, 1, 1, min(12, self.seasonal_period)),
                            ...
                            # MAPE
                            mape = self._calculate_mape(ts, cv_folds)
                            
                            self.models[sku.skuId] = results
                            
                    except Exception as e:
                        logger.warning(f"SARIMAX failed for {sku.skuId}: {e}")
                        forecast_detail = self._simple_forecast(historico, forecast_months)
                        mape = 999
    ...
    def _simple_forecast(self, historico: List[float], periods: int) -> List[float]:
        """Simple moving average fallback"""
        if not historico:
            return [0] * periods
        avg = np.mean([h for h in historico if h > 0]) if any(h > 0 for h in historico) else 0
        return [max(0, avg)] * periods
    
    def _calculate_mape(self, ts: pd.Series, folds: int) -> float:
